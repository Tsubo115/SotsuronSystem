...
public class PlaySkillAudio : MonoBehaviour
{
    public newSkillGen SG;
    ...
    {
        //コルーチン実行中であれば．現在再生中のオーディオ，コルーチンを停止
    ...
    void AddAudioClips(int musicNum)
    {
        ...
        for(int i = 0; i < 8; i++){
            AudioClip clip = Resources.Load<AudioClip>("AudioClips/" + chordName[SG.skillcp[musicNum, i] - 1]); //ChordName[musicNum曲目のi番目のコード]
            audioClips.Add(clip);

            Debug.Log(SG.skillcp[musicNum, i]);
        }
